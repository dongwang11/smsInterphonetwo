...
uint8 K;//The cipher Key.
#define Nb  4//Number of columns (32-bit words) comprising the State. For this standard, Nb = 4.
uint8 Nk;// * Number of 32-bit words comprising the Cipher Key. For this standard, Nk = 4, 6, or 8
uint8 Nr;// Number of rounds, which is a function of  Nk  and  Nb (which is fixed). For this standard, Nr = 10, 12, or 14
...
uint8* aes_init(uint8 *mac_id)
{

	uint8 tmp_key[16] = { 'G', ' ', 'l', ' ', 'o', ' ', 'b', 'W', ' ', 'a', 'l', ' ', 'k', 'i',  ' ', 'e'};
	uint8 id_idx[6] = { 1, 3, 5, 8, 11, 14};

	if(mac_id == NULL)
		return NULL;

	mac_id[0] = 0xff;   //设置地地高字字为0xff;

	for(int i = 0; i < sizeof(id_idx); i++)
	{
		tmp_key[id_idx[i]] = mac_id[i];
	}

	memcpy(aes_key, tmp_key, sizeof(tmp_key));

	return aes_key;
}

// in out 大小为20
void aes_encode(uint8* in, uint8* out)
{
	if(in == NULL || out == NULL)
		return ;
...
// in out 大小为20
void aes_decode(uint8* in, uint8* out)
{
	if(in == NULL || out == NULL)
	{
		//printf("%s \r\n", "aes decoed Null !!");
